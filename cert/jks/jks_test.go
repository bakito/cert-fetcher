...
func Test_exportCerts_cert_0_with_name(t *testing.T) {
	...
	err := exportCerts([]*x509.Certificate{test.NewCert()}, "https://foo.bar", []int{0}, "", "", "file-name.jks")
	assert.NoError(t, err)
...
func Test_exportCerts_to_existing_empty_jks(t *testing.T) {
	...
	err := exportCerts([]*x509.Certificate{test.NewCert()}, "https://foo.bar", nil, "../../testdata/empty.jks", "changeit", "")
	assert.NoError(t, err)
...
func Test_exportCerts_to_existing_jks_duplicate(t *testing.T) {
	...
	err := exportCerts([]*x509.Certificate{test.NewCert()}, "https://foo.bar", nil, "../../testdata/geotrust.jks", "changeit", "")
	assert.NoError(t, err)
...
func Test_exportCerts_to_existing_jks_additional(t *testing.T) {
	...
	err := exportCerts([]*x509.Certificate{test.NewCert()}, "https://foo.bar", nil, "../../testdata/google.jks", "changeit", "")
	assert.NoError(t, err)
